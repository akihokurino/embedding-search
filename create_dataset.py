...
if __name__ == "__main__":
    ...
    file_table_df = create_file_table_df()
    page_table_df = create_page_table_df()

    pdf_files = list(input_dir.glob("*.pdf"))
    for pdf_file in pdf_files:
        file_id = str(uuid.uuid4())
        file_table_df = append_to_file_table(file_table_df, file_id, pdf_file)
        texts = extract_text_from_pdf(pdf_file)
        for i, text in enumerate(texts):
            page_table_df = append_to_page_table(page_table_df, i, file_id, text)
